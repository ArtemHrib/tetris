...
public class ShapeL extends Shape{
	...
	protected boolean shapeTimeMovementY(int mass[][]) {
		if(status == 1||status==2) {
			if(curPosition[0][1]+1<20 && checkYMovement(mass)) {
	...
	protected void shapeMovementX(int direction, int mass[][]) {
		...
		case 2:{
			...
			if(status == 1||status==4) {
				if(curPosition[3][0]+1<10 && checkXMovement(2,mass)) {
					curPosition[0][0]++;
					curPosition[1][0]++;
					curPosition[2][0]++;
					curPosition[3][0]++;
	...
	try{
		if(status == 1) {
			if(mass[curPosition[0][0]+2][curPosition[0][1]] == 0  &&
					mass[curPosition[1][0]+1][curPosition[1][1]+1] == 0  &&
			...
			if(curPosition[3][0]-1>=0) {
				if(mass[curPosition[0][0]][curPosition[0][1]-2] == 0 &&
						mass[curPosition[1][0]+1][curPosition[1][1]-1] == 0  &&
						mass[curPosition[3][0]-1][curPosition[3][1]+1] == 0 ) {
